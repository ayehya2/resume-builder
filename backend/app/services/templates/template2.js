...
const generator = {
  profileSection(basics) {
    ...
    const addressLine = location.address
      ...
      : '';
    const websiteLines = websites.map(website => 
      `{\\faLink\\ \\href{${website.url}}{${website.name}}}`
    );
    const info = [emailLine, phoneLine, addressLine, ...websiteLines]
      .filter(Boolean)
      .join(' | ');
