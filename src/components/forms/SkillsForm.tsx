                ...
                {skills.map((skill, index) => (
                    <div key={index} className="bg-slate-100 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 p-5 space-y-4 shadow-sm transition-all hover:border-slate-300 dark:hover:border-slate-600">
                        <div className="flex justify-between items-center pb-2 border-b border-slate-100 dark:border-slate-700">
                            <h4 className="font-semibold text-slate-800 dark:text-slate-200 uppercase tracking-tight text-sm">Skill Category {index + 1}</h4>
                            <button
                        ...
                        <div>
                            ...
                            <div className="flex gap-2">
                                <input
                                    ...
                                    className="flex-1 px-3 py-2 border-2 border-slate-300 dark:border-slate-600 focus:outline-none focus:ring-2 focus:ring-slate-400/20 focus:border-slate-500 bg-white dark:bg-slate-950 text-slate-900 dark:text-white font-medium transition-all"
                                    placeholder="Type a skill and press Enter"
                                />
                                <button
                                    onClick={() => addSkillItem(index)}
                                    className="px-4 py-2 bg-slate-700 text-white hover:bg-slate-600 font-semibold transition-colors shadow-sm"
                                >
                                    Add
                                </button>
                            </div>
